...
def upgrade():
    ...
    # Only create if they don't already exist (your DB already has them from old migration)
    if "mobile_operations" not in existing_tables:
        op.create_table(
            "mobile_operations",
    ...
    if "mobile_changes" not in existing_tables:
        op.create_table(
            "mobile_changes",
...
def downgrade():
    # make downgrade safe too
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    existing_tables = inspector.get_table_names()
    
    if "mobile_changes" in existing_tables:
        try:
    ...
    if "mobile_operations" in existing_tables:
        try:
